...
int findEulerCycle(Graph graph) {
    int i = 0, currentCheck = 0, newCounter = 0, stabilizationPeriod = 1, initialPeriod = 1;
    Edge* startingPoint = NULL;
    bool unfinished = true;
    
    while (unfinished) {
        currentCheck = graph.returnEdgeFromOverall(i)->getCheck();
        if (currentCheck >= 2) {
            startingPoint = graph.returnEdgeFromOverall(i);
            for (int m = 0; m < graph.getEdgeCount(); m++) {
                if (graph.returnEdgeFromOverall(m) == startingPoint) {
                    initialPeriod = m-1;
                }
            }
            break;
    ...
    startingPoint->incrementCheck();
    
    while (newCounter != graph.getEdgeCount()) {
        graph.insertEdgeToCycle(startingPoint);
        startingPoint = graph.returnEdgeFromOverall(i+1);
        i++;
        newCounter++;
    }
    ...
    cout << endl;
    stabilizationPeriod -= initialPeriod; //Adjust this
